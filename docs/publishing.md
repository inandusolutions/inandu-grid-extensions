...
`Inandu.Grid.Extensions` ships to **nuget.org** as a free (MIT) package. It multi-targets
`net8.0` and `net10.0`.
...
1. Bump `<Version>` in `src/Inandu.Grid.Extensions/Inandu.Grid.Extensions.csproj` (SemVer).
...
4. Verify from a clean tree:
   ...
   dotnet pack  src/Inandu.Grid.Extensions/Inandu.Grid.Extensions.csproj -c Release -o ./artifacts
   ```

5. Inspect the package:

   ```bash
   dotnet nuget verify ./artifacts/Inandu.Grid.Extensions.X.Y.Z.nupkg
...
6. Push the package and its symbols:
   ...
   ```bash
   dotnet nuget push ./artifacts/Inandu.Grid.Extensions.X.Y.Z.nupkg \
     --api-key <KEY> --source https://api.nuget.org/v3/index.json

   dotnet nuget push ./artifacts/Inandu.Grid.Extensions.X.Y.Z.snupkg \
     --api-key <KEY> --source https://api.nuget.org/v3/index.json
   ```
